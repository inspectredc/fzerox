...
s32 __osLeoInterrupt(void) {
    ...
    if (stat & 0x2000000) {
        ...
        IO_WRITE(LEO_BM_CTL, info->bmCtlShadow | 0x1000000);
    ...
    if (info->cmdType == 1) {
        ...
        if (stat & 0x10000000) {
            ...
            __osEPiRawStartDma(__osDiskHandle, 0, LEO_C2_BUFF, blockInfo->C2Addr, blockInfo->sectorSize * 4);
            return 1;
        }
...
static void __osLeoAbnormalResume(void) {
    ...
    IO_WRITE(LEO_BM_CTL, info->bmCtlShadow | 0x10000000);
    pi_stat = IO_READ(PI_STATUS_REG);
    while (pi_stat & (PI_STATUS_DMA_BUSY | PI_STATUS_IO_BUSY)) {
        pi_stat = IO_READ(PI_STATUS_REG);
    }
