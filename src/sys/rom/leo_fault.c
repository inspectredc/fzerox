...
// ASM PROC BUG! 可能性があります。確認して
u8 D_800CD6C8[] = "可能\性があります。確認して";
...
#else
void LeoFault_CopyFontToRam(s32* code, u8* ramAddr, s32* width, s32* height, s32* alignment) {
...
s32 LeoFault_LoadFontSet(u8* codes, s32 fontOffset, s32 fontRamAddr) {
    ...
    for (i = 0; codes[i] != 0; fontOffset += 0x80, i += 2) {
        LeoFault_CopyFontToRam((codes[i] << 8) + codes[i + 1], fontRamAddr + fontOffset);
    }
...
void func_8007F8F8(u32 messageCount, u8** messages, u8* fontRamAddr, s32* offsets) {
    ...
    for (i = 0; i < messageCount; i++) {
        ...
        fontOffset = LeoFault_LoadFontSet(messages[i], fontOffset, fontRamAddr);
    }
}
#else
void LeoFault_LoadFontSet(s32* codes, u8* fontRamAddr, s32* widths, s32* heights, s32* alignments) {
    s32 i;
    s32 fontOffset;

    for (i = 0, fontOffset = 0; codes[i] != 0; fontOffset += 0x80, i++) {
        LeoFault_CopyFontToRam(codes[i], fontRamAddr + fontOffset, &widths[i], &heights[i], &alignments[i]);
    }
}
#endif
...
#else
void LeoFault_LoadFonts(void) {
    LeoFault_LoadFontSet(sLeoFontCodes, gLeoFontBuffer, sLeoFontWidths, sLeoFontHeights, sLeoFontAlignments);
}
#endif

void LeoFault_SetFramebuffer(void) {
    sLeoPrintFrameBuffer = osViGetNextFramebuffer();
}

void LeoFault_DrawErrorBox(void) {
    u8 i;
    u16 j;
...
void LeoFault_DrawErrorMessageNumber(s32 posX, s32 posY, s8* str) {
    s8 var_v1;
    s32 offset;
    s8* var_s1;
...
#else
void LeoFault_DrawErrorMessage(s32 posX, s32 posY, s8* str) {
    s32 fontChar;
    s32 i;
...
#else
    LeoFault_DrawErrorMessage(97, 80, "Error Number");
    LeoFault_DrawErrorMessage(201, 80, errNoStr);
#endif
}
...
#else
    LeoFault_DrawErrorMessage(45, 90, "Caution : Please do not remove");
    LeoFault_DrawErrorMessage(45, 110, "the disk while the access lamp");
    LeoFault_DrawErrorMessage(45, 130, "is blinking.");
#endif
}
...
void LeoFault_DrawCautionDoNotRemovePleaseInsert(void) {
    ...
    LeoFault_DrawErrorMessage(40, 170, LEO_ERROR_MESSAGE_PLEASE);
#else
    LeoFault_DrawErrorMessage(45, 110, "Caution : Please do not remove");
    LeoFault_DrawErrorMessage(45, 130, "the disk while the access lamp");
    LeoFault_DrawErrorMessage(45, 150, "is blinking.");
    LeoFault_DrawErrorMessage(45, 170, "Please insert the disk.");
#endif
}
...
void LeoFault_DrawReinsertDisk(void) {
    ...
    LeoFault_DrawErrorMessage(48, 170, LEO_ERROR_MESSAGE_INSERT_IT_PROPERLY);
#else
    LeoFault_DrawErrorMessage(53, 110, "The eject button may have");
    LeoFault_DrawErrorMessage(53, 130, "been pushed. Please remove");
    LeoFault_DrawErrorMessage(53, 150, "the Disk once and completely");
    LeoFault_DrawErrorMessage(53, 170, "reinsert it.");
#endif
}
...
#else
    LeoFault_DrawErrorMessage(56, 100, "The wrong disk may have");
    LeoFault_DrawErrorMessage(56, 120, "been inserted. Please check.");
#endif
}
...
#else
    LeoFault_DrawErrorMessage(55, 100, "Please insert initial disk");
    LeoFault_DrawErrorMessage(55, 120, "used when turned power ON.");
#endif
}
