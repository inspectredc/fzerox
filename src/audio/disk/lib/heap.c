...
void func_807322D0(f32 p, f32 q, u16* out) {
    s32 i;
    f32 tmp[16];
...
void AudioHeap_Init(void) {
    ...
    if (gAudioCtx.audioBufferParameters.numSequencePlayers > 2) {
        gAudioCtx.audioBufferParameters.numSequencePlayers = 2;
    }
    ...
    gAudioCtx.unk_2040 /= gAudioCtx.audioBufferParameters.aiSamplingFrequency;
    gAudioCtx.unk_2040 /= gAudioCtx.maxTempo;
...
void AudioHeap_UnapplySampleCacheForFont(SampleCacheEntry* entry, s32 fontId) {
    ...
    for (instId = 0; instId < gAudioCtx.soundFontList[fontId].numInstruments; instId++) {
        inst = Audio_GetInstrument(fontId, instId);
        if (inst != NULL) {
...
void AudioHeap_ApplySampleBankCacheInternal(bool apply, s32 sampleBankId) {
    ...
    addrPtr = &change.oldAddr;
    if (apply && (apply == true)) {
        uintptr_t temp = change.newAddr;

        change.newAddr = *addrPtr;
        change.oldAddr = temp;
