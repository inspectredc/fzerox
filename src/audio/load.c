...
void func_800B0268(uintptr_t devAddr, u8* ramAddr, size_t size, s32 medium) {
    ...
    while (true) {
        ...
        func_800B03A0(&gSyncDmaIoMsg, OS_MESG_PRI_HIGH, OS_READ, devAddr, ramAddr, 0x400, &gSyncDmaQueue, medium, D_800D0C14);
    ...
    if (size != 0) {
        func_800B03A0(&gSyncDmaIoMsg, OS_MESG_PRI_HIGH, OS_READ, devAddr, ramAddr, size, &gSyncDmaQueue, medium, D_800D0C20);
        MQ_WAIT_FOR_MESG(&gSyncDmaQueue, NULL);
    }
}
...
void func_800B1140(AudioSlowLoad* slowLoad, s32 size) {
    ...
    func_800B03A0(&slowLoad->ioMesg, OS_MESG_PRI_NORMAL, OS_READ, slowLoad->curDevAddr, slowLoad->curRamAddr, size, &slowLoad->msgQueue,
...
void func_800B15C8(AudioAsyncLoad* asyncLoad, size_t size) {
    ...
    osCreateMesgQueue(&asyncLoad->mesgQueue, &asyncLoad->msg, 1);
    if (size) {}
    func_800B03A0(&asyncLoad->ioMesg, OS_MESG_PRI_NORMAL, OS_READ, asyncLoad->curDevAddr, asyncLoad->curRamAddr, size, &asyncLoad->mesgQueue,
