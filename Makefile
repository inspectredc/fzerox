...
ifeq ($(EXPANSION_KIT),1)
    BUILD_DEFINES   += -DVERSION_JP=1 -DBUILD_REVISION=REVISION_D -DLANGUAGE=LANGUAGE_JPN -DOS_TV_TYPE=OS_TV_TYPE_NTSC -DEXPANSION_KIT -DBUILD_VERSION=VERSION_J -DASSET_VERSION=$(VERSION) -DASSET_REVISION=$(REV_R)
    LEO_VERSION ?= 2
    MFS_VERSION ?= 2
else
    # Version check
    ifeq ($(VERSION),jp)
        BUILD_DEFINES   += -DVERSION_JP=1 -DBUILD_REVISION=REVISION_A -DLANGUAGE=LANGUAGE_JPN -DOS_TV_TYPE=OS_TV_TYPE_NTSC -DBUILD_VERSION=VERSION_I -DASSET_VERSION=$(VERSION) -DASSET_REVISION=$(REV)
    endif

    ifeq ($(VERSION),us)
        BUILD_DEFINES   += -DVERSION_US=1 -DBUILD_REVISION=REVISION_B -DLANGUAGE=LANGUAGE_ENG -DOS_TV_TYPE=OS_TV_TYPE_NTSC -DBUILD_VERSION=VERSION_I -DASSET_VERSION=$(VERSION) -DASSET_REVISION=$(REV)
    endif

    ifeq ($(VERSION),pal)
        BUILD_DEFINES   += -DVERSION_PAL=1 -DBUILD_REVISION=REVISION_C -DLANGUAGE=LANGUAGE_ENG -DOS_TV_TYPE=OS_TV_TYPE_PAL -DBUILD_VERSION=VERSION_I -DASSET_VERSION=$(VERSION) -DASSET_REVISION=$(REV)
        LEO_VERSION ?= 1
        MFS_VERSION ?= 1
        REV := rev0
    endif
