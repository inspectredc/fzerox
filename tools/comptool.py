...
# todo
# file_table_dict = {"US 1.0":0xD9A90}
...
def compress(baserom, comprom, mio0, dma_table=None, verbose=False):
    ...
    (version) = get_version_info(baserom)
    ...
    with open(comprom, 'w+b') as compfile, open(baserom, 'rb') as basefile:
        ...
        while True:
            ...
            file_name = 'segment_' + str(file_count)+'_'+format(v_file_begin, 'X')

            if (file_count in decomp_inds):
...
parser = argparse.ArgumentParser(description='Compress or decompress a F-ZERO X ROM')
parser.add_argument('inROM', help="ROM file to process")
parser.add_argument('outROM', help="Output file for processed ROM.")
parser.add_argument('-c', action='store_true',help='Compress a big endian uncompressed F-ZERO X ROM')
parser.add_argument('-d', action='store_true',help='Decompress a F-ZERO X ROM. Use with -s to also make a big endian compressed ROM.')
parser.add_argument('-e', metavar='extract',dest='extract',help='Directory for extracted decompressed files. Use with -d')
parser.add_argument('-r', action="store_true",help='Fix crc of F-ZERO X ROM without compressing or decompressing')
parser.add_argument('-s', action='store_true',help='Swap a F-ZERO X ROM to big endian (.z64). Use . as second argument to swap in-place or .b to also make a backup')
